...
namespace L1PriceSupport
{
    ...
    {
        private Dictionary<string, KaiTrade.Interfaces.IDOM> _mnemonicDOM;
        private Dictionary<string, L1PriceSupport.PXPublisher> _L1Publisher;
        private Dictionary<string, List<IPriceAgregator>> _priceAgregators;

        public ILog _log = null;

        public MemoryPriceHandler(ILog log)
        {
            _log = log;
        ...
        public KaiTrade.Interfaces.IPublisher CreatePxPub(KaiTrade.Interfaces.IProduct product)
        {
            ...
            {
                pxPub = new L1PriceSupport.PXPublisher(_log);
        ...
        public KaiTrade.Interfaces.IPublisher GetPXPublisher(KaiTrade.Interfaces.IProduct product)
        {
            if (!_L1Publisher.ContainsKey(product.Mnemonic))
            {
                _L1Publisher.Add(product.Mnemonic, new L1PriceSupport.PXPublisher(_log));
            }
        ...
        public KaiTrade.Interfaces.IDOM GetProductDOM(IProduct product, decimal startPx)
        {
            ...
            {
                K2Depth.K2DOM dom = new K2Depth.K2DOM(_log);
